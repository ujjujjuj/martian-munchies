...
export const Navbar = ({ authed }) => {
  return (
    <>
      <div
        className={`flex flex-row justify-between items-center w-full bg-transparent text-white ${styles.nav} px-16 mt-3`}
      >
        <div className="flex flex-row justify-start items-center h-full gap-20">
          ...
          <NavLink
            ...
            exact
            styles={styles}
            className={`${styles.navlink} font-bold text-md hover:text-[#FF7547]`}
          >
            HOME
          </NavLink>
          <NavLink
        ...
        <div className="flex flex-row gap-9 items-center h-full ">
          ...
          {authed && (
            <FiLogOut
              size={24}
              className="cursor-pointer hover:text-[#FF7547]"
              onClick={() => {
